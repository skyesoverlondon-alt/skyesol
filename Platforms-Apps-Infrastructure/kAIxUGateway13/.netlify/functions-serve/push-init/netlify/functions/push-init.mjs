...
// netlify/functions/push-init.js
...
async function createDigestDeploy({ site_id, branch, title, files, netlify_token = null }) {
  const cleanFiles = {};
...
async function getSiteDeploy({ site_id, deploy_id, netlify_token = null }) {
...
async function pollDeployUntil({ site_id, deploy_id, timeout_ms = 6e4, netlify_token = null }) {
  const start = Date.now();
...
var push_init_default = wrap(async (req) => {
  const cors = buildCors(req);
