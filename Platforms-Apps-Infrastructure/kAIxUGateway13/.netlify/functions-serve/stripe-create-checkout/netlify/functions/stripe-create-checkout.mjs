...
var stripe_create_checkout_default = wrap(async (req) => {
  const cors = buildCors(req);
  ...
  const success_url = process.env.STRIPE_SUCCESS_URL || (origin ? `${origin}/?billing=success` : void 0);
  const cancel_url = process.env.STRIPE_CANCEL_URL || (origin ? `${origin}/?billing=cancel` : void 0);
