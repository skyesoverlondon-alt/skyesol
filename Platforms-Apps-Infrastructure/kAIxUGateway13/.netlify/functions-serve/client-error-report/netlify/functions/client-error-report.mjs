...
function safeStr2(v, max = 12e3) {
  if (v == null) return null;
  const s = String(v);
  return s.length <= max ? s : s.slice(0, max) + `\u2026(+${s.length - max} chars)`;
}
function safeJson(obj, max = 2e4) {
  try {
...
var client_error_report_default = wrap(async (req) => {
  const cors = buildCors(req);
  ...
  try {
    ...
    if (raw && raw.length > 25e4) return json(413, { error: "Payload too large" }, cors);
  ...
  const name = safeStr2(err.name || client.name, 300);
  const message = safeStr2(err.message || client.message, 8e3) || "Client error reported";
  const stack = safeStr2(err.stack || client.stack, 2e4);
  const extra = {
    client: safeJson({
