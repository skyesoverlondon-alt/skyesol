...
async function setDeviceRevoked({ api_key_id, install_id, revoked, actor = "admin" }) {
  if (revoked) {
    await q(
      `update key_devices
...
var admin_devices_default = wrap(async (req) => {
  const cors = buildCors(req);
