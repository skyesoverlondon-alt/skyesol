...
async function setNetlifyTokenForCustomer(customer_id, token_plain) {
...
async function clearNetlifyTokenForCustomer(customer_id) {
  await q(`delete from customer_netlify_tokens where customer_id=$1`, [customer_id]);
}

// netlify/functions/admin-netlify-token.js
var admin_netlify_token_default = wrap(async (req) => {
  const cors = buildCors(req);
  ...
  if (req.method === "GET") {
    ...
    const r = await q(`select 1 as ok from customer_netlify_tokens where customer_id=$1`, [customer_id2]);
    return json(200, { customer_id: customer_id2, has_token: r.rows.length > 0 }, cors);
  }
  let body;
  try {
    body = await req.json();
