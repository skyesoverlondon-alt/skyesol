...
// netlify/functions/admin-monitor-events.js
function clampInt(v, def, min, max) {
  const n = parseInt(String(v ?? def), 10);
  if (!Number.isFinite(n)) return def;
  return Math.max(min, Math.min(max, n));
}
var admin_monitor_events_default = wrap(async (req) => {
  const cors = buildCors(req);
  ...
  const limit = clampInt(url.searchParams.get("limit"), 200, 1, 1e3);
  const after_id = clampInt(url.searchParams.get("after_id"), 0, 0, 1e9);
