...
// netlify/functions/_lib/voice_pricing.js
function num(v, def) {
  const n = Number(v);
  return Number.isFinite(n) ? n : def;
}
function voiceMarkupPct() {
  return num(process.env.VOICE_MARKUP_PCT, 31);
}
function voiceRateDollarsPerMinute({ ai = true, telephony = true, recording = true } = {}) {
  const relay = num(process.env.VOICE_AI_RELAY_USD_PER_MIN, 0.07);
  const tel = num(process.env.VOICE_TELEPHONY_USD_PER_MIN, 85e-4);
  const rec = num(process.env.VOICE_RECORDING_USD_PER_MIN, 25e-4);
  return (ai ? relay : 0) + (telephony ? tel : 0) + (recording ? rec : 0);
}
function voiceCostCents(minutes, opts = {}) {
  const rate = voiceRateDollarsPerMinute(opts);
  const cents = Math.round(num(minutes, 0) * rate * 100);
  return Math.max(0, cents);
}
function voiceBillCents(minutes, opts = {}) {
  const cost = voiceCostCents(minutes, opts);
  const markup = voiceMarkupPct() / 100;
  return Math.round(cost * (1 + markup));
}
function voiceMonthKeyUTC() {
  return monthKeyUTC();
}

// netlify/functions/_lib/voice.js
async function updateCallStatus({ provider, callSid, status, durationSeconds }) {
...
async function finalizeBillingForCall(callRow) {
...
// netlify/functions/voice-twilio-status.js
var voice_twilio_status_default = wrap(async (req) => {
