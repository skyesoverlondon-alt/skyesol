...
// netlify/functions/admin-monitor-stream.js
function clampInt(v, def, min, max) {
  const n = parseInt(String(v ?? def), 10);
  if (!Number.isFinite(n)) return def;
  return Math.max(min, Math.min(max, n));
}
var admin_monitor_stream_default = wrap(async (req) => {
  const cors = buildCors(req);
  ...
  let lastId = clampInt(url.searchParams.get("after_id"), 0, 0, 1e9);
...
`));
      ...
      send("hello", { ok: true, after_id: lastId, now: (/* @__PURE__ */ new Date()).toISOString() });
      ...
      timer = setInterval(async () => {
        try {
          ...
          try {
            send("warn", { error: e?.message || String(e) });
          } catch {
          }
        }
      }, 1e3);
