...
var health_default = wrap(async (req) => {
  const cors = buildCors(req);
  if (req.method === "OPTIONS") return new Response("", { status: 204, headers: cors });
  const geminiRaw = process.env.GEMINI_API_KEY;
  const geminiLocalRaw = process.env.GEMINI_API_KEY_LOCAL;
  const norm = (v) => {
    const s = String(v ?? "");
    const trimmed = s.trim();
    return trimmed.replace(/^"(.*)"$/, "$1").trim();
  };
  const geminiNorm = norm(geminiRaw);
  const geminiLocalNorm = norm(geminiLocalRaw);
  const out = {
    ok: true,
    ...
    env: {
      ...
      providers: {
        ...
        gemini_local: !!process.env.GEMINI_API_KEY_LOCAL,
        keyLens: {
          ...
          gemini_local: (process.env.GEMINI_API_KEY_LOCAL || "").length,
          gemini_norm: geminiNorm.length,
          gemini_local_norm: geminiLocalNorm.length,
          gemini_has_quote: String(geminiRaw ?? "").includes('"'),
          gemini_local_has_quote: String(geminiLocalRaw ?? "").includes('"')
        }
      }
    },
    db: { ok: false }
  ...
  try {
    ...
    out.db = { ok: false, error: e?.message || String(e), code: e?.code || void 0, hint: e?.hint || void 0 };
  }
