...
// netlify/functions/admin-github-token.js
var admin_github_token_default = wrap(async (req) => {
  const cors = buildCors(req);
  ...
  if (req.method === "GET") {
    ...
    const customer_id2 = parseInt((url.searchParams.get("customer_id") || "").trim(), 10);
    if (!Number.isFinite(customer_id2)) return badRequest("Missing customer_id", cors);
    const r = await q(`select token_type, scopes from customer_github_tokens where customer_id=$1`, [customer_id2]);
    return json(200, {
