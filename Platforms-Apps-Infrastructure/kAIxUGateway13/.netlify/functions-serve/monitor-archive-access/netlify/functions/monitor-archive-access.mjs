...
// netlify/functions/monitor-archive-access.js
...
var monitor_archive_access_default = wrap(async (req) => {
  const cors = buildCors(req);
  ...
  const cursor = (url.searchParams.get("cursor") || "").trim() || void 0;
  const limit = clampInt(url.searchParams.get("limit"), 200, 10, 2e3);
  const st = store();
  if (key) {
