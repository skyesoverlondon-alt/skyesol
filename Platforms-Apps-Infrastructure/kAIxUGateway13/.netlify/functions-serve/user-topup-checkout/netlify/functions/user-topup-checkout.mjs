...
// netlify/functions/user-topup-checkout.js
var user_topup_checkout_default = wrap(async (req) => {
  const cors = buildCors(req);
  ...
  if (amount_cents > 5e5) return badRequest("Top-up too large", cors);
  ...
  const success_url = process.env.STRIPE_SUCCESS_URL || (origin ? `${origin}/dashboard.html?billing=success` : void 0);
  const cancel_url = process.env.STRIPE_CANCEL_URL || (origin ? `${origin}/dashboard.html?billing=cancel` : void 0);
  if (!success_url || !cancel_url) {
    return json(400, { error: "Missing STRIPE_SUCCESS_URL/STRIPE_CANCEL_URL (or Origin header)" }, cors);
  }
  const session = await stripe.checkout.sessions.create({
    mode: "payment",
    customer_email: keyRow.customer_email || void 0,
    success_url,
    cancel_url,
    line_items: [
      {
