...
// netlify/functions/monitor-archive-prune.js
...
var monitor_archive_prune_default = wrap(async (req) => {
  const cors = buildCors(req);
  ...
  const batchSize = clampInt(process.env.MONITOR_ARCHIVE_BATCH_SIZE, 2e3, 100, 5e3);
  const maxBatches = clampInt(process.env.MONITOR_ARCHIVE_MAX_BATCHES, 6, 1, 20);
  const cutoffTs = Date.now() - retentionDays * 24 * 3600 * 1e3;
